use crate::core::{NodeBuilder, NodeFactory, NodeInfo, NodeInputPorts, NodeOutputPorts};use crate::flow::{FlowMod, FlowNode, parse_flow_all_nodes, parse_flow_file, validate_flow};
...
impl FlowProcessor {
    ...
    pub fn extract_node_types(flow_nodes: &[FlowNode]) -> HashSet<String> {
        flow_nodes.iter().map(|node| node.node_type.clone()).collect()
    }
